...
x2 = pos[0]
y2 = pos[1]
res_mat[x1][y1] = math.sqrt((x2-x1)**2 + (y2-y1)**2) summ = 0
...
res_mat = np.zeros(len(s)*len(s[0]), dtype=float) res_mat = res_mat.reshape(len(s), len(s))
for x1 in range(len(s)):
for y1 in range(len(s[0])):
elem = s[x1][y1]
pos = find_pos(g, elem)
...
x2 = pos[0]
y2 = pos[1]
res_mat[x1][y1] = ((abs(x2-x1)**p) + (abs(y2 -
                                          y1)**p))**(1./p)
summ = 0
for i in range(len(res_mat)):
summ += sum(res_mat[i])
